...
from agentscope.application.ports.trace_read import SessionRecord


class RecordNormalizer(RecordNormalizerPort):
    """Normalise les enregistrements bruts vers le modèle AgentScope."""
    ...
    def _parse_datetime(
        ...
        if not isinstance(value, str):
            issues.append(
                NormalizationIssue(
                    field=target_field,
                    message="La valeur doit être une date ISO-8601.",
                )
            )
            return None
